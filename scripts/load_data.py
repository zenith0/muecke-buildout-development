...
# muecke imports
from muecke.core.models import Action, Shop, Country
from muecke.catalog.models import Category, Product, Image, DeliveryTime
...
from muecke.criteria.models import WeightCriterion, CriteriaObjects, CountryCriterion
from muecke.criteria.settings import LESS_THAN_EQUAL, IS, IS_NOT, GREATER_THAN
from muecke.shipping.models import ShippingMethod, ShippingMethodPrice
from muecke.tax.models import Tax
from portlets.models import PortletAssignment, Slot, PortletRegistration
from muecke.portlet.models import CartPortlet, CategoriesPortlet, PagesPortlet, RecentProductsPortlet, RelatedProductsPortlet, TextPortlet, PagesPortlet, CategoriesPortletTabs
...
def load_data():
    ...
    management.call_command('loaddata', 'muecke_all_countries.xml', verbosity=0)
    ...
    shop, created = Shop.objects.get_or_create(name="muecke test", shop_owner="John Doe", description="Test Shop",
                                               default_country=ie)
    shop.save()
    ...
    for p in products:
        ...
        if i % 3 == 0:
            product.price_calculator = "muecke.gross_price.GrossPriceCalculator"
        else:
            product.price_calculator = "muecke.net_price.NetPriceCalculator"
        product.save()
